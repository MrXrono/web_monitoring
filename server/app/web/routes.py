...
async def server_controllers_partial(
    ...
    async with async_session() as db:
        ...
        mdadm_platform = (srv.last_report or {}).get("mdadm_platform", {}) if srv else {}

    if not ctrls and not mdadm_platform:
        return HTMLResponse(
    ...
    # --- Software RAID platform card (Intel VROC / mdadm) ---
    if mdadm_platform:
        plat_name = mdadm_platform.get("platform") or "Software RAID"
        plat_version = mdadm_platform.get("version") or "N/A"
        raid_levels = mdadm_platform.get("raid_levels") or []
        chunk_sizes = mdadm_platform.get("chunk_sizes") or "N/A"
        max_disks = mdadm_platform.get("max_disks")
        max_volumes = mdadm_platform.get("max_volumes") or "N/A"
        io_controllers = mdadm_platform.get("io_controllers") or 0
        io_controller = mdadm_platform.get("io_controller") or "N/A"
        mdadm_ver = mdadm_platform.get("mdadm_version") or "N/A"
        md_ver = mdadm_platform.get("md_version") or "N/A"
        personalities = mdadm_platform.get("personalities") or []
        ports = mdadm_platform.get("ports") or []

        raid_lvl_badges = " ".join(
            f'<span class="badge bg-primary me-1">{lvl}</span>' for lvl in raid_levels
        ) or "N/A"

        personality_badges = " ".join(
            f'<span class="badge bg-secondary me-1">{p}</span>' for p in personalities
        ) or ""
        ...
        if ports:
            port_rows = []
            for p in ports:
                port_rows.append(f'''<tr>
                  <td class="fw-semibold">{p.get("port", "?")}</td>
                  <td>{p.get("device", "N/A")}</td>
                </tr>''')
            ports_html = f'''
            <div class="card bg-light border mt-3"><div class="card-body p-3">
              <h6 class="card-title small fw-bold mb-2">{_("Attached Ports")}</h6>
              <table class="table table-sm table-borderless mb-0">
                <thead class="table-light"><tr><th>{_("Port")}</th><th>{_("Device")}</th></tr></thead>
        ...
        <div class="card border-0 shadow-sm mb-3">
          <div class="card-header bg-white d-flex justify-content-between align-items-center">
            <h6 class="mb-0">{plat_name}</h6>
            <span class="badge bg-info text-dark">{plat_version}</span>
          </div>
          <div class="card-body">
            <div class="row g-3">
              <div class="col-md-6">
                <table class="table table-sm table-borderless mb-0">
                  <tr><td class="text-muted">{_("Platform")}:</td><td class="fw-semibold">{plat_name}</td></tr>
                  <tr><td class="text-muted">{_("Version")}:</td><td class="fw-semibold">{plat_version}</td></tr>
                  <tr><td class="text-muted">{_("RAID Levels")}:</td><td>{raid_lvl_badges}</td></tr>
                  <tr><td class="text-muted">{_("Chunk Sizes")}:</td><td class="fw-semibold">{chunk_sizes}</td></tr>
                  <tr><td class="text-muted">{_("Max Disks")}:</td><td class="fw-semibold">{max_disks if max_disks is not None else "N/A"}</td></tr>
                  <tr><td class="text-muted">{_("Max Volumes")}:</td><td class="fw-semibold">{max_volumes}</td></tr>
                </table>
              </div>
              <div class="col-md-6">
                <table class="table table-sm table-borderless mb-0">
                  <tr><td class="text-muted">{_("I/O Controllers")}:</td><td class="fw-semibold">{io_controllers}</td></tr>
                  <tr><td class="text-muted">{_("I/O Controller")}:</td><td class="fw-semibold">{io_controller}</td></tr>
                  <tr><td class="text-muted">mdadm:</td><td class="fw-semibold">{mdadm_ver}</td></tr>
                  <tr><td class="text-muted">{_("MD Module")}:</td><td class="fw-semibold">{md_ver}</td></tr>
                  {"<tr><td class='text-muted'>" + _("Personalities") + ":</td><td>" + personality_badges + "</td></tr>" if personality_badges else ""}
                </table>
              </div>
            </div>
            {ports_html}
